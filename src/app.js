...
/**
 * AI-Powered Subheader Detection System
 */

// Known section headers for financial statements
const KNOWN_SECTION_HEADERS = {
  pnl: [
    'revenue', 'revenues', 'sales', 'income',
    'cost of goods sold', 'cogs', 'cost of sales',
    'gross profit', 'gross margin',
    'operating expenses', 'opex', 'operating',
    'selling, general & administrative', 'selling, general and administrative', 'sg&a', 'sga',
    'research & development', 'research and development', 'r&d',
    'operating income', 'ebit', 'ebitda',
    'other income', 'other expenses', 'other',
    'interest', 'taxes',
  ...
  balance: [
    ...
    'equity', 'shareholders\' equity', 'stockholders\' equity', 'owner\'s equity',
    'cash and cash equivalents', 'cash',
    'accounts receivable', 'receivables',
    'inventory', 'inventories',
    'property, plant & equipment', 'property, plant and equipment', 'ppe', 'fixed assets',
...
function calculateSubheaderScore(item, index, allItems, statementType) {
  let score = 0;
  ...
  const values = item.actualValues || [];
  const hasAnyValue = values.some(v => v !== null && v !== undefined && v !== '' && v !== 0);
  const emptyValueCount = values.filter(v => v === null || v === undefined || v === '' || v === 0).length;
  const totalValues = values.length;
  
  // Rule 1: Known section headers (35 points)
  const knownHeaders = KNOWN_SECTION_HEADERS[statementType] || [];
  if (knownHeaders.some(header => nameLower === header || nameLower.includes(header))) {
    score += 35;
  }
  
  // Rule 2: Name length & complexity (20 points max)
  if (wordCount <= 3) {
    score += 20;
  } else if (wordCount <= 4) {
    score += 10;
  ...
  // Rule 3: Capitalization pattern (15 points max)
  if (name === name.toUpperCase() && name.length > 2 && /[A-Z]/.test(name)) {
    score += 15; // ALL CAPS
  } else if (words.every(w => w.length > 0 && w[0] === w[0].toUpperCase())) {
    score += 10; // Title Case
  }
  
  // Rule 4: Position context (25 points max)
  // Check next 5 rows for values
  let nextRowsWithValues = 0;
  for (let i = 1; i <= 5 && index + i < allItems.length; i++) {
    const nextItem = allItems[index + i];
    const nextHasValues = (nextItem.actualValues || []).some(v => v !== null && v !== undefined && v !== '');
    if (nextHasValues) nextRowsWithValues++;
  }
  if (nextRowsWithValues >= 3) score += 25;
  else if (nextRowsWithValues >= 2) score += 15;
  
  // Top 15% of file
  if (index < allItems.length * 0.15) score += 5;
  
  // Rule 5: Value pattern (20 points max)
  if (!hasAnyValue && totalValues > 0) {
    score += 20; // All periods empty
  } else if (emptyValueCount >= totalValues * 0.9) {
    score += 10; // Mostly empty
  } else if (hasAnyValue && emptyValueCount < totalValues * 0.5) {
    score -= 15; // Half or more filled - likely a real line item
  }
  
  // Rule 6: Check if following items contain this item's name (15 points)
  let childCount = 0;
  const keywords = words.filter(w => w.length > 3); // Key words from the name
  for (let i = 1; i <= 5 && index + i < allItems.length; i++) {
    const nextName = (allItems[index + i].name || '').toLowerCase();
    if (keywords.some(kw => nextName.includes(kw))) childCount++;
  }
  if (childCount >= 3) score += 15;
  
  // Rule 7: Anti-patterns (negative scores)
  if (/\d{4}|\d{1,2}\/\d{1,2}/.test(name)) score -= 20; // Contains dates
  if (wordCount >= 8) score -= 15; // Very long name
  if (/product|brand|iphone|android|enterprise|tier/i.test(name)) score -= 10; // Specific products
  if (/accrued|prepaid|deferred|amortization/i.test(name)) score -= 10; // Specific accounts
  
  // Check if it's a total row (should not be a subheader)
  if (/\btotal\b/i.test(name)) score -= 30;
  
  // Clamp score to 0-100
  return Math.max(0, Math.min(100, score));
}

async function detectAndConfirmSubheaders(data) {
  console.log('🔍 Detecting subheaders across all statements...');
  
  const allCandidates = [];
  
  // Analyze each statement type
  ['pnl', 'balance', 'cashflow'].forEach(statementKey => {
    const items = data[statementKey] || [];
    if (items.length === 0) return;
    
    items.forEach((item, index) => {
      const score = calculateSubheaderScore(item, index, items, statementKey);
      
      // Only include if score is uncertain (30-85) - needs user confirmation
      // High confidence (>85) will auto-apply, low (<30) will ignore
      if (score >= 30) {
        allCandidates.push({
  ...
  console.log(`📊 Found ${allCandidates.length} potential subheaders`);
  
  // Separate auto-apply from needs-review
  const autoApply = allCandidates.filter(c => c.autoApply);
  const needsReview = allCandidates.filter(c => !c.autoApply);
  
  // Auto-apply high confidence subheaders
  autoApply.forEach(candidate => {
    setSubheaderOverride(candidate.statement, candidate.name, true);
  });
  
  if (autoApply.length > 0) {
    console.log(`✅ Auto-applied ${autoApply.length} high-confidence subheaders`);
  }
  
  // If there are uncertain cases, use AI for better detection
  if (needsReview.length > 0) {
    console.log(`🤖 Analyzing ${needsReview.length} uncertain items with AI...`);
    await analyzeSubheadersWithAI(needsReview, data);
  }
  
  // Show confirmation modal if there are any items to review
  if (needsReview.length > 0) {
    await showSubheaderConfirmation(needsReview);
  }
}

async function analyzeSubheadersWithAI(candidates, data) {
  if (!window.OPENAI_API_KEY) {
    console.log('⚠️ No OpenAI API key - using rule-based scores only');
    return;
  }
  
  try {
    // Batch all uncertain items into one API call for efficiency
    const prompt = `You are a financial statement analyzer. Review these line items and determine if each is a section subheader or a regular line item.

A SUBHEADER is a category label that groups related line items (e.g., "Revenue", "Operating Expenses", "Current Assets").
A LINE ITEM is an actual financial metric with data (e.g., "Software License Revenue", "Rent Expense", "Office Equipment").

Items to analyze:
${candidates.map((c, i) => `${i + 1}. "${c.name}" (Statement: ${c.statement}, Confidence: ${c.score}%)`).join('\n')}

For each item, respond with ONLY a JSON array like this:
[
  {"index": 1, "isSubheader": true, "confidence": 95},
  {"index": 2, "isSubheader": false, "confidence": 80}
]`;
    ...
    const aiResults = JSON.parse(result.choices[0].message.content.trim());
    
    // Update candidates with AI confidence scores
    aiResults.forEach(ai => {
      const candidate = candidates[ai.index - 1];
      if (candidate) {
        candidate.aiConfidence = ai.confidence;
        candidate.aiSuggestion = ai.isSubheader;
        // Boost score if AI agrees with high confidence
        if (ai.isSubheader && ai.confidence > 80) {
          candidate.score = Math.max(candidate.score, ai.confidence);
        }
      }
    });
...
async function showSubheaderConfirmation(candidates) {
  return new Promise((resolve) => {
    const modal = document.getElementById('subheader-confirmation-modal');
    if (!modal) {
      console.error('Subheader confirmation modal not found');
      resolve();
      return;
    }
    
    const listContainer = document.getElementById('subheader-candidates-list');
    ...
    candidates.forEach((candidate, idx) => {
      const confidenceClass = candidate.score >= 70 ? 'high' : candidate.score >= 50 ? 'medium' : 'low';
      const checked = candidate.score >= 70 ? 'checked' : '';
      
      const itemHTML = `
        <div class="subheader-candidate-item" style="padding: 12px; border-bottom: 1px solid #e9ecef; display: flex; align-items: center; gap: 12px;">
          <input type="checkbox" id="subheader-check-${idx}" ${checked} style="width: 18px; height: 18px; cursor: pointer;">
          <div style="flex: 1;">
            <div style="font-weight: 600; color: #2c3e50; margin-bottom: 4px;">${candidate.name}</div>
            <div style="font-size: 0.85em; color: #6c757d;">
              ${candidate.statement.toUpperCase()} • 
              <span class="confidence-badge confidence-${confidenceClass}" style="padding: 2px 8px; border-radius: 4px; font-weight: 600;">
                ${candidate.score}% confidence
              </span>
              ${candidate.aiConfidence ? ` • AI: ${candidate.aiConfidence}%` : ''}
            </div>
          </div>
        </div>
      `;
    ...
    // Handle confirmation
    const confirmBtn = document.getElementById('confirm-subheaders-btn');
    const newConfirmBtn = confirmBtn.cloneNode(true);
    confirmBtn.parentNode.replaceChild(newConfirmBtn, confirmBtn);
    
    newConfirmBtn.onclick = () => {
      candidates.forEach((candidate, idx) => {
        const checkbox = document.getElementById(`subheader-check-${idx}`);
...
async function processIntegratedForecasting(data) {
  ...
  // STEP 1: Detect subheaders across all statements (structure first!)
  await detectAndConfirmSubheaders(data);
  
  // STEP 2: Detect critical items using pattern matching (NO AI needed for most cases)
  ...
  // STEP 3: Show simplified confirmation UI for mappings
...
async function processBalanceSheetOnly(data) {
  ...
  // STEP 1: Detect subheaders across all statements
  await detectAndConfirmSubheaders(data);
